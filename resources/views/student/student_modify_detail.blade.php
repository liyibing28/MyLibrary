            ...
            <div class="row" >
                <div class="col-sm-5" style="margin-left:350px" >
                    <div class="basic-login" >
                        <form role="form" method="post" action="{{ url('/student/student_modify') }}">
                            {{ csrf_field() }}

                            @foreach ($student->all() as $user)
                            
                            <div class="form-group{{ $errors->has('account') ? ' has-error' : '' }}">
                                <label for="register-username"><i class="icon-user"></i> <b>账号: {{ $user->account }}</b></label>
                                <input class="form-control" id="account" name="account" type="text" placeholder="" value="{{ $user->account }}">
                            ...
                            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                                ...
                                <input class="form-control" id="email" name="email" type="text" placeholder="" value="{{ $user->email }}">
                                @if ($errors->has('email'))
                                    <span class="help-block">
                            ...
                            <div class="form-group{{ $errors->has('class') ? ' has-error' : '' }}">
                                ...
                                <input class="form-control" id="class" name="class" type="text" placeholder="" value="{{ $user->class }}">
                                @if ($errors->has('class'))
                                    <span class="help-block">
                            ...
                            <div class="form-group{{ $errors->has('phone') ? ' has-error' : '' }}">
                                ...
                                <input class="form-control" id="phone" name="phone" type="text" placeholder="" value="{{ $user->phone }}">
                                @if ($errors->has('phone'))
                                    <span class="help-block">
